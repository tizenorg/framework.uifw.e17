...
_e_fm2_dir_load_props_from_parent(const char *path)
{
   ...
   if (((cf) && (cf->dir) && (cf->dir->prop.in_use)) || (parent && !strcmp(parent, path)))
     {
        free(parent);
        return cf;
     }
...
_e_fm2_client_mount(const char *udi, const char *mountpoint)
{
   ...
   strncpy(d, udi, l - 1);
   if (mountpoint)
     strncpy(d + l1 + 1, mountpoint, l - l1 - 1 - 1);
...
/*    } */
   ...
   if (buf[0] == '\0')
     {
        if ((path) && (eina_strlcpy(buf, path, sizeof(buf)) >= sizeof(buf)))
          return NULL;
     }
...
_e_fm2_typebuf_char_append(Evas_Object *obj, const char *ch)
{
   ...
   ts = malloc(len + 1);
   if (!ts) return;
   strncpy(ts, sd->typebuf.buf, len);
   strncat(ts, ch, len - strlen(sd->typebuf.buf));
   free(sd->typebuf.buf);
   sd->typebuf.buf = ts;
   if (ch[0] == '*')
     sd->typebuf.wildcard++;
             ...
             if (ic2->last_selected)
               {
                  ...
                  if (seen)
                    {
                       for (l2 = l, ic2 = l2->data; l2; l2 = l2->prev, ic2 = (l2 ? l2->data : NULL))
                         {
                            if (ic == ic2) break;
                            if (!ic2) continue;
   ...
   if (sd->icon_menu.replace.func)
   ...
   else
     {
        ...
        if (((!(sd->icon_menu.flags & E_FM2_MENU_NO_PASTE)) ||
            ...
            ((sd->realpath) && (ecore_file_can_write(sd->realpath))))
          {
             mi = e_menu_item_new(mn);
             e_menu_item_separator_set(mi, 1);
...
_e_fm_overwrite_rename_yes_cb(void *data, char *text)
{
   ...
   strncat(newpath, text, PATH_MAX - (f - ere->dst) - 1);
...
_e_fm_error_link_source(void *data, E_Dialog *dialog)
{
   ...
   if (!f) goto err;
   if (strlen(ere->dst) + length >= PATH_MAX) goto err;

   newpath[0] = 0;
   strncat(newpath, ere->dst, f - ere->dst);
   if ((strlen(newpath)+strlen(f)) < PATH_MAX)
     {
        strncat(newpath, f, PATH_MAX - (f - ere->dst) - 1);
     }
   else 
     {
        goto err;
     }
   if (e_filereg_file_protected(newpath)) goto err;
   length = 0;
